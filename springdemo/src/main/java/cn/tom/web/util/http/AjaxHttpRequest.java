...
///////////////////////////////////////////////
    ...
    public static void testGet() throws IOException {
        ...
        params.put("getBeans", users);
        String url = "http://127.0.0.1:8080/user/testAjax?" + getUrlParamsByMap(params);
...
//        testGet();
        ...
        Map params = new HashMap();
        List<User> getBeans = new ArrayList<>();
        for (int i = 0; i < 3; i++) {
            User user = new User();
            user.setId(i);
            user.setName("name" + i);
        ...
        params.put("type", "getType");
        params.put("getBeans", getBeans);
        params.put("date", new Date());

        System.out.println(buildParam(params));
    }

    public static String buildParam(Map<String, Object> map) {
                ...
                if (entry.getValue() instanceof String) {
                    ...
                    if (list.size() > 0) {
                        ...
                        if (list.get(0) instanceof String) {
                            sb.append(entry.getKey().getClass().getSimpleName().toLowerCase());
                            sb.append("=[");
                            ...
                            for (int i = 0; i < list.size(); i++) {
                                ...
                                for (int j = 0; j < declaredFields.length; j++) {
                                    ...
                                    if (declaredFields[j].get(list.get(i)) != null) {
                                        sb.append(entry.getKey());
                                        sb.append("[");
                                        sb.append(i);
                                        sb.append("].");
                                        ...
                                        sb.append("=");
                                        sb.append(declaredFields[j].get(list.get(i)));
                                        if (j < declaredFields.length) {
                                            sb.append("&");
                                        }
                                    }
                                }
